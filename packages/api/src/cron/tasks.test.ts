...
describe('cron/tasks.ts runTaskCron', () => {
  ...
  describe('6. per-row isolation', () => {
    ...
    it('all five passes run in one tick and the summary reflects each', async () => {
      ...
      expect(summary).toEqual({ auto_accepted: 1, claims_expired: 0, settle_attempted: 1, settled: 1, expired: 1, recovered: 1, capped: 1, settle_skipped_reason: null, escrow_swept: 0, escrow_stuck: 0 });
