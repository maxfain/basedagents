...
register.post('/init', async (c) => {
  ...
  const challengeBytes = randomBytes(32).toString('base64');
...
register.post('/complete', async (c) => {
  ...
  ).get(challenge_id, agentId) as Challenge | undefined;
  ...
  if (new Date(challenge.expires_at) < new Date()) {
    db.prepare("UPDATE challenges SET status = 'expired' WHERE id = ?").run(challenge_id);
  ...
  // 7. Insert agent + chain entry + mark challenge completed (in a transaction)
  const insertAgent = db.prepare(
    `INSERT INTO agents (id, public_key, name, description, capabilities, protocols, offers, needs, homepage, contact_endpoint, registered_at, status, reputation_score, verification_count)
  ...
  const markChallenge = db.prepare(
  ...
  const txn = db.transaction(() => {
    ...
    markChallenge.run(challenge_id);
  });

  txn();

  // Get the sequence number
  const chainEntry = db.prepare(
    'SELECT sequence FROM chain WHERE entry_hash = ?'
  ).get(entryHash) as { sequence: number };

  // 8. Determine first verification assignment
  // Check active agent count for bootstrap mode
  ...
  if (activeCount.count < 100) {
    ...
    if (profile.contact_endpoint) {
      ...
      // In bootstrap mode, create a self-verification by the registry
      // For now, we'll just mark that bootstrap is needed
      // The actual probe happens asynchronously or via a separate endpoint
    }
  } else {
    ...
    ).get(agentId) as { id: string; contact_endpoint: string | null } | undefined;
